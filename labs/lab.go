...
func initLogger() logrus.Ext1FieldLogger {
	logger := logrus.New()
...
func NewLab(usage string) *cli.App {
	app := cli.NewApp()
	app.EnableBashCompletion = true
	app.Version = "1_1_2024"
	app.Usage = usage
...
func CheckEnvVars(ctx *cli.Context, flags []cli.Flag, prefix string) {
	...
	for _, keyval := range keyvals {
		...
		if flag, ok := known[key]; ok {
			if ctx.Count(flag) > 0 {
				Logger.Info("Config environment variable found", "envvar", key, "shadowedby", "--"+flag)
			} else {
				Logger.Info("Config environment variable found", "envvar", key)
			}
		} else {
			Logger.Warn("Unknown config environment variable", "envvar", key)
		}
	}
}
