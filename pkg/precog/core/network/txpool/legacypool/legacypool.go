...
// Backend defines the minimal set of methods needed to back a tx pool with
// a chain. Exists to allow mocking the live chain out of tests.
type Backend interface {
	// Config retrieves the chain's fork configuration.
	Config() *params.ChainConfig
	Logger() logrus.Ext1FieldLogger
	CurrentBlock() *types.Header                           // CurrentBlock returns the current head of the chain.
	GetBlock(hash common.Hash, number uint64) *types.Block // GetBlock retrieves a specific block, used during pool resets.
	StateAt(root common.Hash) (*state.StateDB, error)      // StateAt returns a state database for a given root hash (generally the head).
}
...
func (config *Config) sanitize() (Config, string) {
	conf := *config
	warning := ""
	if conf.Rejournal < time.Second {
		warning += fmt.Sprint("\n Sanitizing invalid txpool journal time ", "provided ", conf.Rejournal, " updated ", time.Second)
	...
	if conf.PriceBump < 1 {
		warning += fmt.Sprint("\n Sanitizing invalid txpool price bump ", "provided ", conf.PriceBump, " updated ", DefaultConfig.PriceBump)
		conf.PriceBump = DefaultConfig.PriceBump
	}
	if conf.AccountSlots < 1 {
		warning += fmt.Sprint("\n Sanitizing invalid txpool account slots ", "provided ", conf.AccountSlots, " updated ", DefaultConfig.AccountSlots)
	...
	if conf.AccountQueue < 1 {
		warning += fmt.Sprint("\n Sanitizing invalid txpool account queue ", "provided ", conf.AccountQueue, " updated ", DefaultConfig.AccountQueue)
	...
	if conf.Lifetime < 1 {
		warning += fmt.Sprint("\n Sanitizing invalid txpool lifetime ", "provided ", conf.Lifetime, " updated ", DefaultConfig.Lifetime)
...
func New(config Config, backend Backend) *LegacyPool {
	// Sanitize the input to ensure no vulnerable gas prices are set
	warn := ""
	config, warn = (&config).sanitize()
	backend.Logger().WithFields(logrus.Fields{
		"warn": warn,
	}).Warn("Sanitized txpool configuration")

	// Create the transaction pool with its initial settings
	pool := &LegacyPool{
		config:          config,
		backend:         backend,
		signer:          types.LatestSigner(backend.Config()),
...
func (pool *LegacyPool) Init(gasTip *big.Int, head *types.Header, reserve txpool.AddressReserver) error {
	...
	// fully synced).
	statedb, err := pool.backend.StateAt(head.Root)
	if err != nil {
		statedb, err = pool.backend.StateAt(types.EmptyRootHash)
	}
	if err != nil {
		return err
	}
...
func (pool *LegacyPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*sortedMap) {
	...
	if reset != nil {
		...
		if reset.newHead != nil {
			if pool.backend.Config().IsLondon(new(big.Int).Add(reset.newHead.Number, big.NewInt(1))) {
				pendingBaseFee := eip1559.CalcBaseFee(pool.backend.Config(), reset.newHead)
...
func (pool *LegacyPool) reset(oldHead, newHead *types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			var (
				rem = pool.backend.GetBlock(oldHead.Hash(), oldHead.Number.Uint64())
				add = pool.backend.GetBlock(newHead.Hash(), newHead.Number.Uint64())
			)
			if rem == nil {
				...
				for rem.NumberU64() > add.NumberU64() {
					...
					if rem = pool.backend.GetBlock(rem.ParentHash(), rem.NumberU64()-1); rem == nil {
				...
				for add.NumberU64() > rem.NumberU64() {
					...
					if add = pool.backend.GetBlock(add.ParentHash(), add.NumberU64()-1); add == nil {
	...
	if newHead == nil {
		newHead = pool.backend.CurrentBlock() // Special case during testing
	}
	statedb, err := pool.backend.StateAt(newHead.Root)
	if err != nil {
