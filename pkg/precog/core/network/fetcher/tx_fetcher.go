/*
	taken from eth / fetcher / tx_fetcher.go
*/
...
func NewTxFetcher(hasTx func(common.Hash) bool, addTxs func([]*types.Transaction) []error, fetchTxs func(string, []common.Hash) error, dropPeer func(string), logger logrus.Ext1FieldLogger) *TxFetcher {
	return NewTxFetcherForTests(hasTx, addTxs, fetchTxs, dropPeer, mclock.System{}, nil, logger)
}
...
func NewTxFetcherForTests(
	...
	clock mclock.Clock, rand *mrand.Rand, logger logrus.Ext1FieldLogger) *TxFetcher {
	return &TxFetcher{
...
func (f *TxFetcher) Notify(peer string, types []byte, sizes []uint32, hashes []common.Hash) error {
	...
	/*
		f.logger.WithFields(logrus.Fields{
			"peer":   peer,
			"hashes": len(hashes),
			"known":  len(hashes) - len(unknownHashes),
			"new":    len(unknownHashes),
			"dup":    duplicate,
			"cheap":  underpriced,
			"metas":  len(unknownMetas),
		}).Info("TxFetcher Notify")
	*/
	select {
